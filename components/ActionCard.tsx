...
export function ActionCard({ evaluation }: ActionCardProps) {
  ...
  return (
    <div className="action-card">
      ...
      <div className="confidence-bar-bg confidence-bar-container">
        <div className={`confidence-bar-fill ${bgColorClass}`} style={{ width: `${compositeScore}%` }}></div>
      </div>

      {/* NEW GOLD ZONE INFO SECTION */}
      <div className="gold-zone-details">
        <div>
          <div className="metric-header" style={{ fontSize: "0.65rem", textTransform: "uppercase" }}>Recent High ({evaluation.zoneSource === 'AUTO' ? 'Dynamic' : 'Baseline'})</div>
        ...
        <div className="mobile-text-right" style={{ textAlign: "right" }}>
          ...
          <div style={{ fontSize: "0.65rem", color: "var(--text-muted)", marginTop: "0.25rem" }}>
              {zoneSource === "AUTO" ? "Volatility Adjusted" : "Manual Price Entry"}
          </div>
        </div>
        
        <div style={{ gridColumn: "span 2", paddingTop: "0.5rem", borderTop: "1px solid rgba(255,255,255,0.05)" }}>
            <div className="metric-header" style={{ fontSize: "0.6rem", marginBottom: "0.4rem" }}>
              {zoneSource === 'AUTO' ? 'Dynamic Zone Boundaries' : 'Manual Accumulation Zones'}
            </div>
            <div className="zone-pills-container">
               {zoneSource === 'AUTO' ? (
                 <>
                   <div className="zone-pill">Support: {dynamicZones.earlySupport[1]}–{dynamicZones.earlySupport[0]}</div>
                   <div className="zone-pill">Starter: {dynamicZones.starterBuy[1]}–{dynamicZones.starterBuy[0]}</div>
                   <div className="zone-pill">Aggressive: {dynamicZones.aggressiveAdd[1]}–{dynamicZones.aggressiveAdd[0]}</div>
                 </>
               ) : (
                 <>
                   <div className="zone-pill" style={{ color: "#fbbf24" }}>Starter: {evaluation.adjustedConfig?.starterLower}–{evaluation.adjustedConfig?.starterUpper}</div>
                   <div className="zone-pill" style={{ color: "#fbbf24" }}>Aggressive: {evaluation.adjustedConfig?.aggressiveLower}–{evaluation.adjustedConfig?.aggressiveUpper}</div>
                 </>
               )}
            </div>
        </div>
